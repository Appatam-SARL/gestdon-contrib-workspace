...
const ImageUploader = ({ value, onChange,multiple = false }) => {
    ...
    return (
        ...
        <input type="file" accept="image/*" onChange={handleChange} multiple={multiple} className="mb-2" />
        <div className="flex flex-wrap gap-4">
          {previews.map((src, index) => (
            <div key={index} className="relative">
              <img src={src} alt={`Logo ${index}`} className="h-20 object-contain border rounded" />
              <button
                type="button"
